  ...
  return (
    <section className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="text-lg font-semibold" style={{ color: "var(--secondary)" }}>MODULE 4: International Benchmarking Report</div>
        <span className="rounded px-2 py-1 text-xs" style={{ backgroundColor: '#EEF2FF', color: 'var(--secondary)' }}>Q3 2025</span>
      </div>
      <div className="rounded-xl border bg-white shadow-sm p-5" style={{ borderColor: "var(--fifth)" }}>
        <div className="flex items-center justify-between">
          <div className="text-sm font-medium" style={{ color: "var(--secondary)" }}>ESG Implementation Comparison</div>
          <div className="text-[10px]" style={{ color: 'var(--fourth)' }}>Bars show score out of 100</div>
        </div>
        <div className="mt-4 grid gap-3 text-sm" style={{ color: "var(--secondary)" }}>
          {rows.map((row) => (
            <div key={row.region} className="grid sm:grid-cols-5 gap-2 items-center">
              <div className="font-medium">{row.region}</div>
              {[row.overall, row.e, row.s, row.g].map((score, i) => (
                <div key={i} className="h-2 rounded-full bg-[color:var(--secondary)]/15 relative">
                  <div className="absolute inset-y-0 left-0 rounded-full" style={{ width: `${score}%`, backgroundColor: 'var(--secondary)' }} />
                </div>
              ))}
            </div>
          ))}
        </div>
        <div className="mt-3 grid grid-cols-5 gap-2 text-[10px]" style={{ color: 'var(--fourth)' }}>
          <div></div>
          <div>Overall</div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border bg-white shadow-sm p-5 space-y-4" style={{ borderColor: 'var(--fifth)' }}>
          <div className="text-sm font-medium" style={{ color: 'var(--secondary)' }}>Gap Analysis: Abu Dhabi vs. Leaders</div>
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <div className="text-xs font-medium mb-1" style={{ color: 'var(--fourth)' }}>Strengths (on par or better)</div>
              <ul className="list-disc pl-5 text-sm space-y-1" style={{ color: 'var(--secondary)' }}>
            ...
            <div>
              <div className="text-xs font-medium mb-1" style={{ color: 'var(--fourth)' }}>Opportunities for improvement</div>
              <ul className="list-disc pl-5 text-sm space-y-1" style={{ color: 'var(--secondary)' }}>
                <li>Environmental: -18 pts vs. Denmark — renewable energy, circular economy</li>
                <li>Social: -19 pts vs. Denmark — diversity & employee wellbeing</li>
                <li>Data depth for Scope 3 supplier categories</li>
              </ul>
            </div>
          </div>
          <div className="grid sm:grid-cols-3 gap-3 text-xs">
            <div className="rounded border p-3" style={{ borderColor: 'var(--fifth)', color: 'var(--secondary)' }}>
              <div className="font-medium">Priority themes</div>
              <div className="mt-2 flex flex-wrap gap-2">
                {['Renewables','Diversity','Logistics S3','Water'].map(t => (
                  <span key={t} className="rounded px-2 py-1" style={{ backgroundColor: '#EEF2FF', color: 'var(--secondary)' }}>{t}</span>
                ))}
              </div>
            </div>
            <div className="rounded border p-3" style={{ borderColor: 'var(--fifth)', color: 'var(--secondary)' }}>
        ...
        <div className="rounded-xl border bg-white shadow-sm p-5 space-y-4" style={{ borderColor: 'var(--fifth)' }}>
          <div className="text-sm font-medium" style={{ color: 'var(--secondary)' }}>Best Practices from Global Leaders</div>
      ...
      <div className="flex flex-wrap gap-3">
        <button className="h-10 px-4 rounded-lg border text-sm" style={{ borderColor: "var(--secondary)", color: "var(--secondary)" }}>Download Full Report</button>
        <button
